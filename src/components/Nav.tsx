...
export default function Nav({ navLinks, currentPath, theme, toggle }: Props) {
  ...
  function toggleMenu() { playClick(); setOpen(o => !o) }
  ...
  return (
    <>
      ...
      >
        <button onClick={() => go('/')} style={{ display: 'flex', alignItems: 'center', gap: 10, background: 'none', border: 'none', position: 'relative', zIndex: 610 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 16, position: 'relative', zIndex: 610 }}>
          <div style={{ opacity: open ? 0 : 1, transition: 'opacity 0.2s', pointerEvents: open ? 'none' : 'all' }}>
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            style={{ position: 'fixed', top: 0, left: 0, right: 0, zIndex: 590, background: C.dark, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}
          >
            <div style={{ position: 'absolute', inset: 0, pointerEvents: 'none', backgroundImage: `repeating-linear-gradient(90deg,rgba(26,86,240,0.04) 0,rgba(26,86,240,0.04) 1px,transparent 1px,transparent 80px),repeating-linear-gradient(0deg,rgba(26,86,240,0.04) 0,rgba(26,86,240,0.04) 1px,transparent 1px,transparent 80px)` }} />

            <nav style={{ position: 'relative', zIndex: 1, padding: '80px 10vw 40px', flex: 1, overflowY: 'auto' }}>
              {navLinks.map((n, i) => {
                const isHov    = hovered === n.path
                ...
                return (
                  ...
                  >
                    <span style={{ fontFamily: F.mono, fontSize: '0.6rem', color: 'rgba(245,245,245,0.28)', letterSpacing: '0.12em', minWidth: 24, transition: 'opacity 0.2s', opacity: otherHov ? 0.1 : 1 }}>
                      0{i + 1}
                    </span>
                    <motion.button
                      onClick={() => go(n.path)}
                      onHoverStart={() => setHover(n.path)}
                      onHoverEnd={() => setHover(null)}
                      animate={{ x: isHov ? 18 : 0, opacity: otherHov ? 0.08 : 1, color: isHov ? C.blue : currentPath === n.path ? C.blue : C.darkText }}
                      transition={{ duration: 0.2, ease: [0.16, 1, 0.3, 1] }}
                      style={{ fontFamily: F.clash, fontWeight: 700, fontSize: 'clamp(2rem,6vw,5.5rem)', lineHeight: 1.0, letterSpacing: '-0.03em', color: currentPath === n.path ? C.blue : C.darkText, background: 'none', border: 'none', textAlign: 'left', display: 'block', padding: '3px 0' }}
                    >
                      {n.label}
                    </motion.button>
                  </motion.div>
