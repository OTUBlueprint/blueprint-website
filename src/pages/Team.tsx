...
export default function Team({ theme, go }: Props) {
  ...
  return (
    <div style={{ padding: '100px 52px 80px' }}>
      ...
      {TEAM.map((m, i) => {
        const accent = ROLE_COLORS[m.role] || '#1A56F0'
        return (
          ...
          <Reveal>
            <div>
              <div style={{ fontFamily: F.clash, fontWeight: 700, fontSize: 'clamp(2rem,4vw,3.4rem)', lineHeight: 0.95, letterSpacing: '-0.04em', color: t.fg, marginBottom: 18 }}>
                7 roles<br /><span style={{ color: '#1A56F0' }}>open now.</span>
              </div>
              ...
              <button
                onClick={() => go('apply')}
                style={{ fontFamily: F.syne, fontWeight: 700, fontSize: '0.72rem', letterSpacing: '0.1em', textTransform: 'uppercase', background: '#1A56F0', color: '#fff', padding: '11px 26px', borderRadius: 10, border: 'none', transition: 'background 0.2s, transform 0.15s' }}
                onMouseEnter={e => { const el = e.currentTarget as HTMLElement; el.style.background = '#2D6BFF'; el.style.transform = 'translateY(-1px)' }}
                onMouseLeave={e => { const el = e.currentTarget as HTMLElement; el.style.background = '#1A56F0'; el.style.transform = '' }}
              >
                View All Roles
              </button>
            </div>
          ...
          <Reveal delay={80}>
            <div>
              {['Developer','Product Designer','Illustrator / Motion Designer','Social Media Manager','Finance Officer','Event Coordinator','Sponsorship Associate'].map(role => (
                <motion.div key={role} whileHover={{ x: 5 }}
                  ...
                  <span style={{ fontFamily: F.mono, fontSize: '0.6rem', letterSpacing: '0.1em', textTransform: 'uppercase', color: '#1A56F0' }}>Open →</span>
                </motion.div>
              ))}
            </div>
          </Reveal>
      ...
      <Reveal>
        <div style={{ fontFamily: F.mono, fontSize: '0.58rem', letterSpacing: '0.2em', textTransform: 'uppercase', color: t.fg3, margin: '44px 0 20px', paddingBottom: 14, borderBottom: `1px solid ${t.bord}` }}>
          Faculty advisors
        </div>
      </Reveal>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 2 }}>
        {[1,2,3].map(i => (
          <Reveal key={i} delay={i * 50}>
            <div style={{ border: `1.5px dashed ${t.bord}`, borderRadius: 10, padding: '22px 20px', opacity: 0.52 }}>
              <div style={{ fontFamily: F.syne, fontWeight: 700, fontSize: '0.82rem', color: t.fg, marginBottom: 8 }}>Faculty advisor slot open</div>
              <p style={{ fontFamily: F.mono, fontSize: '0.72rem', color: t.fg3, lineHeight: 1.62, fontWeight: 400 }}>
                Seeking faculty from Engineering, Science, or Business. Reach out at blueprintotech@gmail.com
              </p>
            </div>
          </Reveal>
        ))}
